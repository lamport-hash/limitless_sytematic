"""
Normalize all firstrate Stocks daily data files.

This script processes all .txt files in data/import/firstrate/stocks_daily/
...
def normalize_all_stocks_daily(p_test_single: bool = False):
    """Normalize all Stocks daily files from firstrate.
    
    Args:
        p_test_single: If True, only process one file for testing.
    """
    
    subtype = "stocks_daily"
    source = ExchangeNAME.FIRSTRATE
    data_freq = MktDataTFreq.CANDLE_1DAY
    product_type = ProductType.STOCKS

    import_dir = Path(__file__).parent.parent.parent / "data" / "import" / "firstrate" / "stocks_daily"
    ...
    for txt_file in tqdm(files_to_process, desc="Normalizing"):
        ...
        try:
            df = norm_firstdata(str(txt_file), "/tmp/temp_norm_stocks.df.parquet")
...
if __name__ == "__main__":
    ...
    normalize_all_stocks_daily(p_test_single=test_mode)
