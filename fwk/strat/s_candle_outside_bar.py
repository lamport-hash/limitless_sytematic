"""
Outside Bar Candlestick Pattern Strategy.

Strategy that detects Outside Bar patterns and trades with ATR-based SL/TP.
- Long Signal: Bearish engulfing bar with close below previous low
- Short Signal: Bullish engulfing bar with close above previous high
"""
...
def build_features(
    ...
    """
    Build Outside Bar signal features and ATR.
    ...
    bdf.add_feature(FeatureType.OUTSIDE_BAR_SIGNAL)
...
class OutsideBarStrategy(Strategy):
    """
    Outside Bar pattern strategy with ATR-based stop loss and take profit.
...
def run_backtest(
    ...
    **kwargs
    ...
    """
    Run backtest with Outside Bar strategy.
...
def main(
    ...
    if p_verbose:
        print("=" * 80)
        print("Outside Bar Candlestick Strategy - QQQ ETF")
        print("=" * 80)
    ...
    df = build_features(df, p_direction=p_direction, p_atr_len=p_atr_len)
    
    n_long = df["F_outside_bar_long_f16"].sum()
    n_short = df["F_outside_bar_short_f16"].sum()
