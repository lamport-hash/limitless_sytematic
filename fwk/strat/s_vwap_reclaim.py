...
Strategy that trades VWAP crossovers with volume confirmation.
- Long: Price crosses above VWAP from below with volume
...
def build_features(
    ...
    p_vol_threshold: float = 1.0,
) -> pd.DataFrame:
    """Build VWAP Reclaim features and signals."""
    df = p_df.copy()
    ...
    long_entry = (prev_close < prev_vwap) & (df[g_close_col] > df["vwap"])
    short_entry = (prev_close > prev_vwap) & (df[g_close_col] < df["vwap"])
    
    if p_vol_filter:
        vol_filter = df["vol_ratio"].fillna(0) > p_vol_threshold
        long_entry &= vol_filter
        short_entry &= vol_filter
...
class VWAPReclaimStrategy(Strategy):
    """VWAP Reclaim strategy with ATR-based SL/TP."""
...
def run_backtest(
    ...
    **kwargs
    ...
    """Run backtest with VWAP Reclaim strategy."""
