"""
First 5 Bias Strategy.

Strategy that trades pullbacks to OR5 boundary in candle direction.
- Long: OR5 bullish + price pulls back near or5_high
- Short: OR5 bearish + price bounces near or5_low
"""
...
def build_features(
    ...
    p_pullback_pct: float = 0.30,
) -> pd.DataFrame:
    """Build First 5 Bias features and signals."""
    df = p_df.copy()
    ...
    import datetime
    in_window = (times >= datetime.time(9, 35)) & (times <= datetime.time(11, 0))
    
    bull5 = df["or5_close"] > df["or5_open"]
    bear5 = df["or5_close"] < df["or5_open"]
    
    band = p_pullback_pct / 100.0
    
    near_high = (
        (df[g_close_col] >= df["or5_high"] * (1.0 - band)) &
        (df[g_close_col] <= df["or5_high"] * (1.0 + band))
    )
    near_low = (
        (df[g_close_col] >= df["or5_low"] * (1.0 - band)) &
        (df[g_close_col] <= df["or5_low"] * (1.0 + band))
    )
    
    long_entry = bull5 & near_high & in_window
    short_entry = bear5 & near_low & in_window
...
class First5BiasStrategy(Strategy):
    """First 5 Bias strategy with ATR-based SL/TP."""
...
def run_backtest(
    ...
    **kwargs
    ...
    """Run backtest with First 5 Bias strategy."""
...
def main(
    ...
    if p_verbose:
        print("=" * 80)
        print(f"First 5 Bias Strategy - Direction: {p_direction}")
        print("=" * 80)
