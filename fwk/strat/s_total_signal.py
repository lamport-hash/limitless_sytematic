"""
Total Signal Pattern Strategy.

Strategy that detects 5-candle continuation/breakout patterns and trades with ATR-based SL/TP.
- Long Signal: Ascending lows with bullish breakout (Close > prev High)
- Short Signal: Descending highs with bearish breakout (Close < prev Low)
"""
...
def build_features(
    ...
    """
    Build Total Signal features and ATR.
    ...
    bdf.add_feature(FeatureType.TOTAL_SIGNAL)
...
class TotalSignalStrategy(Strategy):
    """
    Total Signal pattern strategy with ATR-based stop loss and take profit.
...
def run_backtest(
    ...
    **kwargs
    ...
    """
    Run backtest with Total Signal strategy.
...
def main(
    ...
    if p_verbose:
        print("=" * 80)
        print("Total Signal Pattern Strategy - QQQ ETF")
        print("=" * 80)
    ...
    n_long = df["F_total_signal_long_f16"].sum()
    n_short = df["F_total_signal_short_f16"].sum()
