...
Pure price action strategy that trades breakouts of Donchian channels.
...
def build_features(
    ...
    p_channel_period: int = 20,
) -> pd.DataFrame:
    """Build Donchian Channel Breakout features and signals."""
    df = p_df.copy()
    ...
    long_entry = df[g_close_col] > prev_upper
    short_entry = df[g_close_col] < prev_lower
...
def convert_to_ohlcv(p_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ohlcv_df["dc_upper"] = p_df["dc_upper"]
    ohlcv_df["dc_lower"] = p_df["dc_lower"]
...
class DonchianBreakoutStrategy(Strategy):
    """Donchian Channel Breakout strategy with ATR-based SL/TP."""
...
def run_backtest(
    ...
    **kwargs
    ...
    """Run backtest with Donchian Breakout strategy."""
...
def main(
    ...
    if p_verbose:
        print("=" * 80)
        print(f"Donchian Breakout Strategy - {p_channel_period} period - Direction: {p_direction}")
        print("=" * 80)
    ...
    df = build_features(
        ...
        p_channel_period=p_channel_period,
    )
