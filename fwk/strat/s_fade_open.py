...
Strategy that fades an over-extended open back toward VWAP.
- Short: Gap up + RSI overbought + bearish candle
- Long: Gap down + RSI oversold + bullish candle
"""
...
def build_features(
    ...
    p_min_gap_pct: float = 0.50,
    p_rsi_overbought: float = 65.0,
    p_rsi_oversold: float = 35.0,
    p_fade_window_mins: int = 10,
) -> pd.DataFrame:
    """Build Fade Open features and signals."""
    df = p_df.copy()
    
    bdf = BaseDataFrame(p_df=df)
    bdf.add_feature(FeatureType.GAP_PCT)
    bdf.add_feature(FeatureType.VWAP)
    bdf.add_feature(FeatureType.RSI, periods=[14])
    ...
    import datetime
    end_minutes = 9 * 60 + 30 + p_fade_window_mins
    end_h, end_m = divmod(end_minutes, 60)
    fade_end = datetime.time(end_h, end_m)
    
    market_open = datetime.time(9, 30)
    in_window = (times >= market_open) & (times <= fade_end)
    
    bull_bar = df[g_close_col] > df[g_open_col]
    bear_bar = df[g_close_col] < df[g_open_col]
    
    long_entry = (df["gap_pct"] <= -p_min_gap_pct) & (df["rsi14"] < p_rsi_oversold) & bull_bar & in_window
    short_entry = (df["gap_pct"] >= p_min_gap_pct) & (df["rsi14"] > p_rsi_overbought) & bear_bar & in_window
...
class FadeOpenStrategy(Strategy):
    """Fade Open strategy with ATR-based SL/TP."""
...
def run_backtest(
    ...
    **kwargs
    ...
    """Run backtest with Fade Open strategy."""
...
def main(
    ...
    if p_verbose:
        print("=" * 80)
        print(f"Fade Open Strategy - Direction: {p_direction}")
        print("=" * 80)
    ...
    df = build_features(
        ...
        p_rsi_overbought=p_rsi_overbought,
        p_rsi_oversold=p_rsi_oversold,
    )
